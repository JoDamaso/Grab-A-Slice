...
function getPizza() {
  ...
  fetch(`/api/pizzas/${pizzaId}`)
    ...
    .catch(err => {
      ...
      // this method works aslong as this session has a previous page, "Back" button clicked
      window.history.back();
    });
};

function printPizza(pizzaData) {
  // object from our fetch, response = pizzaData in this function
...
function htmlReplies(replies) {
  const isReplies = (replies && replies.length) 
  if (!isReplies) {
    return '<h5 class="p-1">No replies yet!</h5>'
  } 
  

  return `<h5>${replies.length} ${
    replies.length === 1 ? 'Reply' : 'Replies'
    }</h5>
  ${replies.map(printReply).join('')}`
};
